...
pub enum LazorkitError {
    ...
    /// Invalid role permission value
    InvalidRolePermission,
    /// Account data was modified unexpectedly (snapshot verification failed)
    AccountDataModifiedUnexpectedly,
    /// CPI to unauthorized program (not in whitelist)
    UnauthorizedCpiProgram,
    /// Insufficient balance (below rent exemption minimum)
    InsufficientBalance,
}
