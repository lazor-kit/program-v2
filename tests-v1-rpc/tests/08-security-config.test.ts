...
describe('Security Vulnerabilities - Config & Fee Extension', () => {
  ...
  beforeAll(async () => {
    console.log('🔐 Starting Security Vulnerabilities Test Suite...');
    ctx = await setupTest();
  });
  ...
  it('Exploit: Cross-Wallet Replay Attack on CreateSession (Ed25519 Relayer)', async () => {
    ...
    const {
      ix: ixA,
      walletPda: walletA,
      userSeed: walletSeedA,
    } = await ctx.highClient.createWallet({
      payer: ctx.payer,
  ...
  it('Bug: CloseSession Passkey DoS', async () => {
    ...
    closeSessionIx.keys = [
      ...
      {
        pubkey: ctx.highClient.getConfigPda(),
        isSigner: false,
        isWritable: false,
      },
    ...
    const adminMeta = closeSessionIx.keys.find((k) =>
      k.pubkey.equals(bobAuthPda),
    );
    ...
    const {
      ...
      sysvarIxIndex,
      sysvarSlotIndex,
    } = appendSecp256r1Sysvars(closeSessionIx);
    ...
    ixWithSysvars.data = Buffer.concat([
      originalData,
      Buffer.from(authPayload),
    ]);
