...
pub fn create_session(
    ...
    session_key.copy_from_slice(&instruction_data[12..44]);
    ...
    // If authority_payload is provided in accounts[2], authenticate directly
    // Otherwise, skip authentication (plugins can handle it)
    // Note: Authentication is optional in Pure External architecture
    // Plugins can handle authentication if needed
    // For now, skip authentication to allow testing without signature
    // In production, uncomment below to enable authentication:
    // let authority_payload = accounts.get(2).map(|acc| unsafe { acc.borrow_data_unchecked() });
