...
/// Authenticate authority for an operation
/// 
/// In Pure External architecture, authentication can be:
/// 1. Direct authentication (if authority_payload provided in accounts)
/// 2. Plugin-based authentication (handled by plugins)
/// 
/// This function handles direct authentication if authority_payload is available.
/// If not available, authentication is skipped (optional in Pure External).
pub fn authenticate_authority(
    ...
    match authority_type {
        AuthorityType::Ed25519 => {
            ...
            let mut authority = ED25519Authority::from_create_bytes(&authority_data.authority_data)?;
        ...
        AuthorityType::Ed25519Session => {
            ...
            let authority_ref = unsafe {
                Ed25519SessionAuthority::load_unchecked(&authority_bytes)?
            };
            // Copy using ptr::read (safe for Copy types, but we need it for non-Copy)
            let mut authority: Ed25519SessionAuthority = unsafe { 
                core::ptr::read(authority_ref as *const Ed25519SessionAuthority) 
            };
        ...
        AuthorityType::Secp256k1Session => {
            ...
            let authority_ref = unsafe {
                Secp256k1SessionAuthority::load_unchecked(&authority_bytes)?
            };
            // Copy using ptr::read
            let mut authority: Secp256k1SessionAuthority = unsafe { 
                core::ptr::read(authority_ref as *const Secp256k1SessionAuthority) 
            };
            authority.authenticate_session(accounts, authority_payload, data_payload, slot)?;
        },
        ...
        AuthorityType::Secp256r1Session => {
            ...
            authority_bytes.copy_from_slice(&authority_data.authority_data[..88]);
            let authority_ref = unsafe {
                Secp256r1SessionAuthority::load_unchecked(&authority_bytes)?
            };
            // Copy using ptr::read
            let mut authority: Secp256r1SessionAuthority = unsafe { 
                core::ptr::read(authority_ref as *const Secp256r1SessionAuthority) 
            };
            authority.authenticate_session(accounts, authority_payload, data_payload, slot)?;
        },
        AuthorityType::ProgramExec => {
            ...
            let mut authority = ProgramExecAuthority::new(program_id_bytes, instruction_prefix_len);
            authority.authenticate(accounts, authority_payload, data_payload, slot)?;
        },
        AuthorityType::ProgramExecSession => {
            ...
            authority_bytes.copy_from_slice(&authority_data.authority_data[..80]);
            let authority_ref = unsafe {
                ProgramExecSessionAuthority::load_unchecked(&authority_bytes)?
            };
            // Copy using ptr::read
            let mut authority: ProgramExecSessionAuthority = unsafe { 
                core::ptr::read(authority_ref as *const ProgramExecSessionAuthority) 
            };
            authority.authenticate_session(accounts, authority_payload, data_payload, slot)?;
        },
        AuthorityType::None => {
