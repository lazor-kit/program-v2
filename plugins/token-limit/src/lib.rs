...
//!
//! This plugin enforces token transfer limits per authority, similar to Swig's
//! TokenLimit action. It tracks remaining token amounts that can be transferred
...
fn handle_initialize(
    ...
    let mint = Pubkey::try_from(mint_bytes.as_ref())
    ...
    let config_bytes = unsafe {
        core::slice::from_raw_parts(&config as *const TokenLimitConfig as *const u8, TokenLimitConfig::LEN)
    };
