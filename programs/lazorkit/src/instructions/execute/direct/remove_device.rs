...
use crate::instructions::RemoveDeviceArgs;
...
pub fn remove_device<'c: 'info, 'info>(
    ...
    require!(
        args.policy_data.get(0..8) == Some(&sighash("global", "remove_device")),
...
#[derive(Accounts)]
#[instruction(args: RemoveDeviceArgs)]
pub struct RemoveDevice<'info> {
    ...
    #[account(
        mut,
        seeds = [WalletDevice::PREFIX_SEED, &create_wallet_device_hash(smart_wallet.key(), args.remove_credential_hash)],
        bump,
        owner = ID,
        close  = smart_wallet,
    )]
    pub remove_wallet_device: Box<Account<'info, WalletDevice>>,
